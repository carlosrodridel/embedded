...
#define WIFI_SSID "COOKIE 1003_NORMAL"
#define WIFI_PASS "30127580"
...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            esp_mqtt_client_subscribe(event->client, "t1carlos", 0);
            break;
...
void main_task(void)
{
    ...
    while(1){
    	esp_mqtt_client_publish(client, "t2carlos", "mqtt_esp", 0, 2, 0);
